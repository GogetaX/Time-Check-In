...
func InitCountryButton():
	for x in $Country.get_children():
		if not x is Timer:
			x.connect("index_pressed", Callable(self, "SelectCountry").bind(x))

func LoadSettings():
	var S = GlobalSave.GetValueFromSettingCategory("SalaryDeduction")
	if S == null:
		custom_minimum_size.y = ClosedOpenedYValue.y
...
func EnableDisableModule(Enable):
	var T = create_tween()
	if Enable:
		T.tween_property(self, "custom_minimum_size:y", ClosedOpenedYValue.x, 0.2).set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_OUT)
	else:
		T.tween_property(self, "custom_minimum_size:y", ClosedOpenedYValue.y, 0.2).set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_OUT)
	T.connect("finished", Callable(self, "FinishAnim"))

func FinishAnim():
	pass  # No manual cleanup needed in Godot 4.x
...
func SelectGender(GenderName):
	for x in $GenderSelector.get_children():
		if x.name == GenderName:
			...
			return
	# If none selected, deactivate both
