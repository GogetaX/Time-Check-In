...
func _ready():
	...
	CurSelectedMonth = Time.get_datetime_dict_from_system()
	InitMonthButtons()
...
func InitMonthButtons():
	...
	for x in get_children():
		if x is Button:
			...
			x.connect("pressed", Callable(self, "MonthBtnPressed").bind(x))
	...
	if BtnNode is Button:
		...
		match BtnNode:
			...
			"Export":
				...
				return
			_:
				print("Error TotalCurMonth.gd->MonthBtnPressed() String unknown: ", BtnNode)
				return
	else:
		print("Error TotalCurMonth.gd->MonthBtnPressed() ButtonType unknown: ", BtnNode)
...
func DisplayElements(_Date):
	...
	Info = {"title": "total_period", "desc": str(minmax)}
	...
	if DaysInMonth != null:
		for x in DaysInMonth:
			...
			if DaysInMonth[x].has("check_in1"):
				TotDays += 1
	Info = {"title": "total_worked_days", "desc": str(TotDays)}
	Itm.ShowItem(Delay, Info)
	Delay += 0.1
	...
	if working_hours.has("monthly_hours"):
		...
		Info["desc"] += "/" + str(working_hours["monthly_hours"])
	Itm.ShowItem(Delay, Info)
	Delay += 0.1
	var Gross = 0
	var Sufix = ""
	...
	if TotDays > 0:
		...
		if working_hours.has("hours") && working_hours.has("minutes"):
			...
			Info["progress_percent"] = float(Info["desc"]) * 100.0 / float(str(working_hours["hours"]) + "." + str(minutes))
			Info["desc"] += "/" + str(working_hours["hours"]) + "." + str(minutes)
		Itm.ShowItem(Delay, Info)
		Delay += 0.1
			...
			if Deduction["country"] == "Israel":
				...
				if Rest.has("NosafotHours 125%") || Rest.has("NosafotHours 150%"):
					if Rest.has("NosafotHours 125%"):
						Itm = TotalItemOvertimeInstance.instantiate()
						VBox.add_child(Itm)
		...
		for x in DaysInMonth:
			if DaysInMonth[x].has("report"):
				if DaysInMonth[x]["report"] == "Day Off":
					TotDays += 1
	Info = {"title": "total_days_off", "desc": str(TotDays)}
	Itm.ShowItem(Delay, Info)
	Delay += StepTime
		...
		for x in DaysInMonth:
			if DaysInMonth[x].has("report"):
				if DaysInMonth[x]["report"] == "Holiday":
					TotDays += 1
	Info = {"title": "total_holidays", "desc": str(TotDays)}
	Itm.ShowItem(Delay, Info)
	Delay += StepTime
...
func DisplayMonth(Date):
	...
	if MonthIndicator != "":
		text = TranslationServer.translate("total_title").format([MonthIndicator, GlobalTime.GetMonthName(Date["month"])[1], str(Date["year"])])
	else:
		text = GlobalTime.GetMonthName(Date["month"])[1] + " - " + str(Date["year"])
