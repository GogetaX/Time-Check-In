...
func InitAllCurrencyButtons():
	for x in $Currency.get_children():
		if not x is Timer:
			x.connect("index_pressed", Callable(self, "SelectCurrency").bind(x))
...
# Updated to accept toggle value from signal
func _on_CheckBox_OnToggle(val):
	DisableEnable(!val)
	GlobalSave.AddVarsToSettings("SaloryCalculation", "enabled", val)
	...
	if OldSalary > 0 && NewVar > OldSalary:
		...
		if PercentUp >= 1 && PercentUp <= 100:
			...
			if MonthsUsed != null && MonthsUsed.size() > 1:
				PercentUp = str(PercentUp) + "%"  # Updated to str() for consistency
				var PopupData = {
					"type": "Congrats",
					"Title": TranslationServer.translate("salary_raise"),
					"Desc": TranslationServer.translate("congrats_on_salary_raise") % PercentUp
				}
...
func SyncFromSave():
	...
	if S.has("enabled"):
		$Activate.AnimToggle(S["enabled"])  # Set initial state explicitly
		DisableEnable(!S["enabled"])

func _on_CurrentCurrencyIcon_gui_input(event):
	if !$Activate.is_Pressed:  # Kept as is; adjust if is_Pressed isn’t a property
		return
